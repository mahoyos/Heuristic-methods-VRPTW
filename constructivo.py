...
def find_best_insertion(route, unvisited, nodes_dict, capacity_left, current_time):
    best_node = None
    ...
    return best_node, best_increase, best_new_time
...
def construct_routes(n, Q, nodes_dict):
    ...
    while unvisited:
        ...
        while True:
            best_node, best_increase, best_new_time = find_best_insertion(route, unvisited, nodes_dict, capacity_left, current_time)
            if best_node is None:
                break

            last_node = route[-1]
            route_distance += nodes_dict[last_node]['distances'][best_node]
            route.append(best_node)
            unvisited.remove(best_node)
            capacity_left -= nodes_dict[best_node]['demand']
            current_time = best_new_time

            return_to_depot_time = current_time + nodes_dict[best_node]['distances'][0]
            if return_to_depot_time > nodes_dict[0]['time_window'][1]:
                route.pop()
...
def main():
    ...
    save_to_excel(results_dict, "VRPTW_Miguel_Hoyos_constructivo.xlsx")
