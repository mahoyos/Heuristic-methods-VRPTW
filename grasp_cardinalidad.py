...
def find_insertion(route, unvisited, nodes_dict, capacity_left, current_time, N=3):
    feasible_nodes = []
    ...
    for node in unvisited:
        ...
        increase = nodes_dict[last_node]['distances'][node] + nodes_dict[node]['distances'][0]

        feasible_nodes.append((node, increase, new_time))

    if not feasible_nodes:
        return None, float('inf'), None

    # Sort the feasible nodes by the increase in cost (distance)
    feasible_nodes.sort(key=lambda x: x[1])

    # Select the top N nodes (or all if there are fewer than N)
    top_N_nodes = feasible_nodes[:N]

    # Randomly choose one from the top N
    chosen_node, increase, new_time = random.choice(top_N_nodes)

    return chosen_node, increase, new_time
...
def construct_routes(n, Q, nodes_dict):
    ...
    while unvisited:
        ...
        while True:
            node, increase, new_time = find_insertion(route, unvisited, nodes_dict, capacity_left, current_time)
            if node is None:
                break
...
def main():
    ...
    save_to_excel(results_dict, "VRPTW_Miguel_Hoyos_grasp-cardinalidad.xlsx")
